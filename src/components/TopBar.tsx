...
export default function TopBar() {
    ...
    return (
        <header className="h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-40 flex items-center justify-between px-4 md:px-8 gap-4">
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="relative" ref={dropdownRef}>
                    <button
                        onClick={() => setOpen(o => !o)}
                        className={`flex items-center gap-2.5 px-2.5 py-1.5 rounded-xl transition-colors ${open ? 'bg-gray-100 dark:bg-gray-800' : 'hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                    >
                        ...
                        <div className="text-left hidden sm:block">
                            <p className="text-xs font-semibold text-gray-800 dark:text-gray-200 leading-none">{user?.name || 'Usuario'}</p>
                            <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 leading-none">{user?.orgName || 'Organización'}</p>
                        </div>
                    ...
                    {/* Dropdown menu */}
                    {open && (
                        <div className="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 py-2 z-50 animate-in fade-in zoom-in-95 duration-100">
                            {/* User info header */}
                            ...
                            <div className="px-2 pt-1.5">
                                ...
                                >
                                    ...
                                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Integraciones</span>
                                </button>
                            </div>

                            <div className="mx-2 my-1.5 border-t border-gray-100 dark:border-gray-700" />

                            <div className="px-2 pb-1.5">
                                <button
                                    onClick={handleLogout}
                                    className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors text-left group"
                                >
                                    <LogOut size={15} className="text-gray-400 group-hover:text-red-500 transition-colors" />
                                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-red-600 transition-colors">Cerrar Sesión</span>
                                </button>
                            </div>
                        </div>
                    )}
