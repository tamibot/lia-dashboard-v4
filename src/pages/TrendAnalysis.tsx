...
import { getProfile, getCursos, getProgramas, getWebinars, getGeminiKey } from '../lib/storage';
import { analyzeTrends } from '../lib/gemini';
import { TrendingUp, Sparkles, Loader, ArrowLeft } from 'lucide-react';
...
export default function TrendAnalysisPage() {
    ...
    async function handleAnalyze() {
        if (!getGeminiKey()) { setResult('⚠️ Configura tu API Key de Gemini.'); return; }
        if (!profile) { setResult('⚠️ Completa tu perfil primero.'); return; }
        setLoading(true);
        setResult('');
        try {
            const offerings = { cursos_libres: cursos, programas, webinars };
            ...
            <div className="card mb-6">
                <div className="flex-between">
                    ...
                    <button className="btn btn-primary" onClick={handleAnalyze} disabled={loading}>
                        {loading ? <><Loader size={16} /> Analizando...</> : <><Sparkles size={16} /> Analizar Tendencias</>}
                    </button>
                </div>
            </div>
