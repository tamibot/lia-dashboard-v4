            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 animate-fade-in">
                    ...
                    <div className="mb-6">
                        ...
                        {members.length === 0 ? (
                            <div className="text-center py-6 text-gray-400 border-2 border-dashed border-gray-200 rounded-lg text-sm">
                                        ...
                                        <div key={i} className="border border-gray-200 rounded-lg overflow-hidden">
                                            <div className="flex items-center justify-between p-3 bg-gray-50 cursor-pointer hover:bg-gray-100"
                                                ...
                                                <div className="flex items-center gap-3 min-w-0">
                                                    ...
                                                    {m.role && <span className="text-[10px] text-blue-600 bg-blue-50 px-2 py-0.5 rounded">{m.role}</span>}
                                                    {m.email && <span className="text-xs text-gray-400 hidden md:inline truncate">{m.email}</span>}
                                                </div>
                                            ...
                                            {isExp && (
                                                <div className="p-4 bg-white space-y-4 border-t border-gray-200">
                                                    ...
                                                    <div className="grid grid-cols-3 gap-4">
                                                        <div>
                                                            <label className="text-[10px] font-bold text-gray-500 uppercase">Horario</label>
                                                        ...
                                                        <div>
                                                            <label className="text-[10px] font-bold text-gray-500 uppercase">Inicio vacaciones</label>
                                                        ...
                                                        <div>
                                                            <label className="text-[10px] font-bold text-gray-500 uppercase">Fin vacaciones</label>
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        <button type="button" onClick={() => updateMember(i, { isAvailable: !m.isAvailable })} disabled={isSaving}
                                                            className={`flex items-center gap-2 text-sm font-medium ${m.isAvailable ? 'text-green-700' : 'text-red-600'}`}>
                    ...
                    <div className="mb-6">
                        <label className="block text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                            ...
                            <span className="text-xs font-normal text-gray-400">({productAssignments.length})</span>
                        </label>
                        {productsByType.length === 0 ? (
                            <p className="text-sm text-gray-400 italic">No hay productos disponibles.</p>
                        ) : (
                            <div className="space-y-3 max-h-[250px] overflow-y-auto border border-gray-200 rounded-lg p-4 bg-gray-50">
                                {productsByType.map(group => (
                                    <div key={group.value}>
                                        <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">{group.label}</div>
                                                ...
                                                return (
                                                    <button key={product.id} type="button" onClick={() => toggleProduct(product.entityType, product.id)} disabled={isSaving}
                                                        className={`text-xs px-3 py-1.5 rounded-lg border transition-all disabled:opacity-50 ${
                                                            selected ? `${group.color} border-current font-semibold ring-1 ring-current` : 'bg-white text-gray-600 border-gray-200 hover:border-gray-400'
                                                        }`}>
                                                        {product.title}
                                                    </button>
                                                );
                    ...
                    <div className="pt-4 border-t flex justify-end gap-3">
                        <button onClick={resetForm} className="btn btn-ghost text-gray-600" disabled={isSaving}>Cancelar</button>
                ...
                <>
                    {teams.length === 0 ? (
                        <div className="py-20 text-center bg-white rounded-xl border border-dashed border-gray-200">
                            ...
                            <p className="text-gray-500 mb-4">Comienza creando tu primer equipo comercial.</p>
                                ...
                                <div key={team.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                                    ...
                                    <div className="px-5 py-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                                        <div className="flex items-center gap-3">
                                            ...
                                            {team.description && <span className="text-xs text-gray-400">— {team.description}</span>}
                                    ...
                                    {team.members && team.members.length > 0 ? (
                                            ...
                                            <tbody>
                                                {team.members.map((m, i) => {
                                                    ...
                                                    return (
                                                        <Fragment key={rowId}>
                                                        <tr
                                                            className={`border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer transition-colors ${isExp ? 'bg-blue-50/30' : ''}`}
                                                            ...
                                                            <td className="px-5 py-3">
                                                                <div className="flex items-center gap-2.5">
                                                                    ...
                                                                    <span className="text-sm font-semibold text-gray-900">{m.name || 'Sin nombre'}</span>
                                                                </div>
                                                            </td>
                                                            ...
                                                            <td className="px-5 py-3 hidden lg:table-cell">
                                                                {m.role && <span className="text-[10px] bg-blue-50 text-blue-700 px-2 py-0.5 rounded font-medium">{m.role}</span>}
                                                            </td>
                                                            <td className="px-5 py-3">
                                                                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                                                                    onVacation ? 'bg-amber-100 text-amber-700' :
                                                                    m.isAvailable ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'
                                                                }`}>
                                                        ...
                                                        {isExp && (
                                                            <tr key={`${rowId}-detail`}>
                                                                <td colSpan={6} className="px-5 py-4 bg-gray-50/50 border-b border-gray-100">
                                                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                                                                        <div>
                                                                            <span className="text-gray-400 font-semibold uppercase text-[10px]">Email</span>
                                                                            <p className="text-gray-700 mt-0.5 flex items-center gap-1"><Mail size={11} /> {m.email || '—'}</p>
                                                                        </div>
                                                                        <div>
                                                                            <span className="text-gray-400 font-semibold uppercase text-[10px]">Teléfono</span>
                                                                            <p className="text-gray-700 mt-0.5 flex items-center gap-1"><Phone size={11} /> {m.phone || '—'}</p>
                                                                        </div>
                                                                        <div>
                                                                            <span className="text-gray-400 font-semibold uppercase text-[10px]">WhatsApp</span>
                                                                            <p className="text-gray-700 mt-0.5 flex items-center gap-1"><MessageCircle size={11} /> {m.whatsapp || '—'}</p>
                                                                        </div>
                                                                        <div>
                                                                            <span className="text-gray-400 font-semibold uppercase text-[10px]">Horario</span>
                                                                            <p className="text-gray-700 mt-0.5 flex items-center gap-1"><Clock size={11} /> {m.availability || '—'}</p>
                                                                        </div>
                                                                        {(m.vacationStart || m.vacationEnd) && (
                                                                            <div>
                                                                                <span className="text-gray-400 font-semibold uppercase text-[10px]">Vacaciones</span>
                                                                        ...
                                                                        {m.specialties && m.specialties.length > 0 && (
                                                                            <div>
                                                                                <span className="text-gray-400 font-semibold uppercase text-[10px]">Especialidades</span>
                                                                        ...
                                                                        {m.maxLeads && (
                                                                            <div>
                                                                                <span className="text-gray-400 font-semibold uppercase text-[10px]">Max Leads</span>
                                                                                <p className="text-gray-700 mt-0.5">{m.maxLeads}</p>
                                                                            </div>
                                                                        )}
                                                                    </div>
                                                                </td>
                                        ...
                                        <div className="px-5 py-4 text-sm text-gray-400 italic">
                                            No hay miembros en este equipo. <button onClick={() => handleEdit(team)} className="text-blue-600 hover:underline">Agregar</button>
                                        </div>
                                    )}

                                    {/* Product assignments footer */}
                                    {team.productAssignments && team.productAssignments.length > 0 && (
                                        <div className="px-5 py-2.5 bg-gray-50/50 border-t border-gray-100 flex items-center gap-2 flex-wrap">
                                            <span className="text-[10px] text-gray-400 font-semibold uppercase">Productos:</span>
